...
class SanphamController extends Controller
{
    ...
    public function postAdd(SanphamAddRequest $request)
    {
        ...
        $request->file('txtSPImage')->move(
            base_path() . '/public/resources/upload/sanpham/', $filename
        );
    	$sanpham = new Sanpham;
        ...
        foreach ($files as $file) {
            if(!empty($file)){
                ...
                $file->move(
                    base_path().'/public/resources/upload/chitietsanpham/', $filename
                );
    ...
    {   
        ...
        foreach ($chitiet as $val) {
            $image = 'public/resources/upload/chitietsanpham/'.$val->hinhsanpham_ten;
            File::delete($image);
            DB::table('hinhsanpham')->where('sanpham_id',$id)->delete();
        }
    	$sanpham = DB::table('sanpham')->where('id',$id)->first();
        $img = 'public/resources/upload/sanpham/'.$sanpham->sanpham_anh;
        File::delete($img);
    ...
    public function postEdit($id, SanphamEditRequest $request)
    {
        ...
        $img_current = 'public/resources/upload/sanpham/' . $request->input('fImageCurrent');
        if ($request->hasFile('fImage')) {
             $filename = $request->file('fImage')->getClientOriginalName();
             $sanpham->sanpham_anh = $filename;
             $request->file('fImage')->move(public_path('resources/upload/sanpham/'), $filename);
             File::delete($img_current);
        } else {
            echo "File empty";
        }
    
        if ($request->hasFile('fEditImage')) {
            foreach ($request->file('fEditImage') as $file) {
                ...
                if (isset($file)) {
                    ...
                    $file->move(public_path('resources/upload/chitietsanpham/'), $file->getClientOriginalName());
                    $detail_img->save();
                } 
          }
        }
    ...
    public function delImage($id){
        if (Request::ajax()) {
            ...
            if(!empty($image_detail)) {
                $img = 'public/resources/upload/chitietsanpham/'.$image_detail->hinhsanpham_ten;
                //print_r($img);
                //if(File::isFile($img)) {
                    File::delete($img);
                //}
